...
export function RequireRole({ allow, children }: { allow: LaceRole[]; children: ReactNode }) {
  const { user } = useAuth();
    ...
    return (
      <div className="editorial-panel rounded-[var(--radius-card)] p-8 text-center">
        <h2 className="section-title text-lg text-[color:var(--ink)]">
          View not available for this role
        </h2>
        <p className="mt-2 text-sm font-medium text-[color:var(--ink-muted)]">
          You are signed in as{" "}
          <strong className="text-[color:var(--brand)]">
            {user ? getAccessLabel(user.userType) : "Learner"}
          </strong>
