...
const LOGIN_ERROR_MESSAGES: Record<string, string> = {
  ...
  google_denied: "Google declined the sign-in request. Please try again.",
...
type LoginFormProps = {
  /** Set server-side via HUB_LOGIN_PROVIDER — see docs/adr/0012. */
  loginProvider: "brightspace" | "google";
};

export function LoginForm({ loginProvider }: LoginFormProps) {
  ...
  const oauthStartPath =
    loginProvider === "google" ? "/api/auth/google/start" : "/api/auth/brightspace/start";
  const oauthStartHref = returnTo
    ? `${oauthStartPath}?returnTo=${encodeURIComponent(returnTo)}`
    : oauthStartPath;
        ...
        <div className="relative w-full max-w-md">
          ...
          <div className="editorial-panel rounded-[var(--radius-card)] bg-[color:var(--surface)] px-8 pb-8 pt-7">
              ...
              <p className="mt-2 text-sm font-medium text-[color:var(--ink-muted)]">
                {isDemoMode
                  ...
                  : loginProvider === "google"
                    ? "Sign in with your MLRI Google account for staff preview access."
            ...
            {!isDemoMode ? (
              ...
              >
                {loginProvider === "google"
                  ? "Sign in with your MLRI Google account"
