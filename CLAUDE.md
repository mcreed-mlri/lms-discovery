...
- `react-hooks/set-state-in-effect` is intentionally off in `eslint.config.mjs` (localStorage hydration pattern); see the comment there before re-enabling. It is the **only** disabled rule, and the repo has **zero** `eslint-disable` comments and **zero** `any`/`as any`/`@ts-ignore` under `strict: true` — keep all three properties.
